...
///
/// ⚠ <b>자동 문턱(Otsu, <see cref="CvBlobOpt.UseOtsu"/> — 0.29.0 부터 기본 꺼짐)은 영역에 두 무리가 있을 때만 맞다.</b>
/// 한 무리뿐이면(제품 없음·제품이 영역을 채움) 잡음·조명 기울기를 반으로 갈라, 반쯤 켜진 잡음 화소가 8-연결로
/// 이어져 큰 블랍 하나가 된다 — 점 잡음인데도 조각나지 않아 MinArea 로 안 걸러진다.
/// 실측(영역 200×200, 배경 30 / 재료 220, 잡음 균일 ±5·±10, 조명 기울기 ±20): 제품 없는 영역에서 <b>영역의 42~54%
/// 블랍</b>(고정 문턱은 없음), 영역을 덮은 제품은 면적이 절반 안팎(42~54%, 고정 문턱은 전부), 잡음이 없으면 문턱 0 이 되어 극성에 따라
/// 영역 전체 또는 없음. 두 무리가 있으면(영역의 0.8% 인 작은 제품, 기울기 위의 제품 포함) 자동도 고정도 맞았다.
/// 밴드 점유율로 판정하는 모양(40×628, 어두운 극성, 미디언 3, MinArea 50, 합격 기준 점유율 20%)에서는 대상이 빠진
/// 밴드를 49~66% 로 읽어 합격시켰다(고정 문턱 100 은 0%). <b>있음·없음이나 면적 하한으로 판정할 때는 고정 문턱을 쓴다.</b>
/// </summary>
