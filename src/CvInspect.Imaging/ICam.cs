...
public interface ICam : IDisposable
{
    ...
    ///
    /// <b>구독자는 이 안에서 같은 카메라를 동기로 되부르지 않는다.</b> 단발 그랩이 낸 프레임은 그랩이
    /// 카메라의 락을 쥔 동안 다른 스레드에서 발행될 수 있어, 되부르면 그 락을 기다리며 서로를 붙잡는다 —
    /// 그랩의 시한은 프레임을 <b>이미 받은 뒤</b>라 그 교착을 풀지 못하고, 그 카메라의 닫기·재연결·앱 종료
    /// 까지 함께 멈춘다. UI 로 넘길 때 <b>동기 마샬링</b>도 같은 이유로 걸린다: 그랩을 부른 것이 UI
    /// 스레드였다면 그 스레드가 그랩에 묶여 있어 마샬링이 영영 돌아오지 않는다. 비동기로 넘긴다.</summary>
