...
/// ⚠ 0.29.0(취득 라이브러리 0.4.1)부터 리더가 알린 것보다 짧게 끝난 블록은 불완전으로 센다. 취득을 멈출 때 전송 중이던
/// 블록을 끊는 기종이면 멈출 때마다(라이브 정지, 시한 초과·취소로 끝난 단발 그랩) 두 계수가 함께 오른다 — 손실이 아니다.
/// 실측(Basler acA2500-14gm 한 대): 부른 뒤 몇 ms 안에 취소하는 그랩을 20번씩 몰아 부르면 잘린 블록이 나왔고, 그런 묶음 8번 중
/// 2번은 **바로 다음 정상 그랩의 장까지** 끊겼다(취득 라이브러리 0.4.0·0.4.1 둘 다). 평범한 시한 초과는 끊지 않았다. 다른 기종은 안 쟀다.
/// 뒤의 측정에서 잘린 것은 취소된 그랩 자신의 블록이 아니라(758회 중 0 — 온전히 오거나 아예 안 왔다) <b>그 직후에 건 시작의
/// 블록</b>이었다. 0.29.1 부터 GevCam 은 장을 못 받고 끝난 그랩 뒤의 다음 시작(단발·연속 모두)을 잠깐 미룬다 — 규칙과 실측은
/// Gev 패키지 README 의 Health 절. 두 계수가 멈출 때나 취소 직후에만 오르는지 먼저 본다.
/// </summary>
