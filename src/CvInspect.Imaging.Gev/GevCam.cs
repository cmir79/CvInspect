    ...
    private async Task ApplyExposureAsync(GenApiNodeMap nodes, double timeUs, CancellationToken ct)
    {
        ...
        catch (GenApiException ex) when (TryReadGrid(ex, out var anchor, out var increment))
        {
            ...
            try
            {
                ...
                //
                // ⚠ **거절하는 것은 장치가 아니라 이 스택이다.** 카메라 XML 이 그 정수 레지스터에 선언한
                // `Inc` 를 GenApi 검증이 **쓰기 전에** 강제해 던지는 것이고, 장치는 그 값을 본 적도 없다
                // (`IntegerNodeBase.ValidateAsync` — `inc > 1` 일 때만 격자를 본다. 실수 노드는 격자 검사를
                // 아예 안 한다). 그러니 이 길이 도는 조건은 **장치의 성질이 아니라 "XML 이 1보다 큰 Inc 를
                ...
                // 대상을 따라가면 **장치 포트에 정적 주소를 가진 읽기 전용 레지스터**다(실측: 이 한 대).
        ...
        // 조건은 **셋 다**여야 한다 — ① `Cachable != NoCache` ② 폴링 주기가 없을 것
        // (①②가 GevSharp `RegisterCore.IsCacheable`. 폴링이 붙은 레지스터는 장치가 스스로 값을 바꾸는
        // 것이라 캐시 경로를 통째로 끈다 — 주기로 만료되는 게 아니다) ③ **쓸 수 있을 것** — `WriteThrough`
        // 캐시는 **쓸 때** 채워지므로 읽기 전용 노드에서는 "우리가 쓴 값" 이 애초에 캐시에 들어가지 않는다.
        // 그래서 `Invalidate()` 를 넣지 않았다.
        //
        // ⚠ ③이 없다고 캐시가 안 도는 것은 아니다 — **읽기도 캐시를 채운다.** 읽기 전용이어도 ①②만
        // 맞으면 첫 읽기가 캐시에 남아 **장치가 바꾼 값을 못 본다**(그래서 래치 읽기는 캐시를 버리고 읽는다).
        // 두 갈래를 섞지 않는다: ③은 **우리가 쓴 값이 돌아오는** 메아리의 조건이고, ①②는 **낡은 값이
        // 돌아오는** 것의 조건이다.
        // 지금까지 본 두 대가 **각각 다른 절**에서 빠진다: Basler acA2500-14gm 은 `NoCache`(직접 실측),
        // Crevis MG-A320K-35 는 `WriteThrough` 지만 `PollingTimeMs=1000`(소비자 관측 한 대). 둘 다
        // 되읽기가 진짜 장치 값이다.
        //
