    ...
    private async Task ApplyExposureAsync(GenApiNodeMap nodes, double timeUs, CancellationToken ct)
    {
        ...
        catch (GenApiException ex) when (TryReadGrid(ex, out var anchor, out var increment))
        {
            ...
            try
            {
                ...
                //
                // 이 개체는 그 `<Integer>` 의 Inc 가 리터럴이 아니라 **pInc 로 다른 노드를 가리킨다.**
                // 그 대상이 장치 레지스터면 격자 선언 자체가 **장치가 그때 답한 값**이 되는데, 대상의
                // 노드 종류는 **확인하지 않았다**(이름만 그렇게 보인다 — pInc 는 계산 노드나 상수도 가리킬
                // 수 있다). 격자가 장치에서 오는지 XML 에 박혀 있는지를 근거로 무엇을 판단하려면 **그때
                // 노드 종류부터 확인한다.**
                //
        ...
        // 조건은 둘 다여야 한다 — 노드가 `Cachable != NoCache` 이고 **그리고** 폴링 주기가 없을 것
        // (GevSharp `RegisterCore.IsCacheable`. 폴링이 붙은 레지스터는 장치가 스스로 값을 바꾸는 것이라
        // 캐시 경로를 통째로 끈다 — 주기로 만료되는 게 아니다). 그래서 `Invalidate()` 를 넣지 않았다.
