...
| `ClipOverlayToImage` | `bool` | Clip `Overlay` to the drawn image (default `false`: graphics run to the edge of the display area, so a label on the image border stays readable in the fit margin). Turn it on where a cut-out is shown (`ViOverlay.CropTo`) — otherwise geometry crossing the cut and labels outside it are drawn in the margin, as if the image went on. `Shapes` are never clipped. |
